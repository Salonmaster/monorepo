...
void Helpers::KeyDeriver::deriveKeys(const std::string& masterSecret, std::string& outAesKey, std::string& outHmacKey) {
    ...
    if (EVP_PKEY_derive_init(pctx) <= 0 || EVP_PKEY_CTX_set_hkdf_md(pctx, EVP_sha256()) <= 0 ||
        EVP_PKEY_CTX_set1_hkdf_salt(pctx, reinterpret_cast<const unsigned char*>(salt.data()), salt.size()) <= 0 ||
        EVP_PKEY_CTX_set1_hkdf_key(pctx, reinterpret_cast<const unsigned char*>(masterSecret.data()),
                                   masterSecret.size()) <= 0 ||
        EVP_PKEY_CTX_add1_hkdf_info(pctx, nullptr, 0) <= 0 || EVP_PKEY_derive(pctx, output, &outLen) <= 0) {
