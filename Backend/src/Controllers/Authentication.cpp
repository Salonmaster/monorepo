...
void Controllers::Authentication::sessionState(const drogon::HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback) const {
    auto session = req->attributes()->get<std::shared_ptr<drogon_model::backend::Sessions>>("session");
    auto resp = HttpResponse::newHttpResponse();
    resp->setStatusCode(k200OK);
    resp->setContentTypeCode(drogon::CT_APPLICATION_JSON);
    resp->setBody(*glz::write_json(Schemas::Error {
        *session->getAccessToken()
    }));
    callback(resp);
}

void Controllers::Authentication::authenticate(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback) const {
    ...
    Helpers::LoginUrlParams params{
        Application::Globals::instance().keycloakServerUrl,
...
void Controllers::Authentication::handleCallback(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback) const {
    ...
    codeVerifierMapper.findByPrimaryKey(state,
    [code, callback, dbClient, req](const drogon_model::backend::CodeVerifiers &verifier) mutable {
        ...
        auto client = HttpClient::newHttpClient(Application::Globals::instance().keycloakServerUrl, app().getLoop(), false, false);
        client->sendRequest(keycloakReq, [req, callback, dbClient, verifier](ReqResult result, const HttpResponsePtr &response) {
            if (result == ReqResult::Ok && response) {
                ...
                Helpers::KeyDeriver::deriveKeys(Application::Globals::instance().masterSecret.unlock(),
                                                 aesKey, hmacKey);
    ...
    [callback](const drogon::orm::DrogonDbException &e) {
        ...
        response->setBody(*glz::write_json(Schemas::Error {
            "Failed to find code verifier."
        }));
        callback(response);
    });  
    }
