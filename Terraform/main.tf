...
terraform {
  backend "s3" {
    ...
    key = "infra.tfstate" # overridden in CI

    endpoints = {
      s3 = "https://salonmaster-ci.nbg1.your-objectstorage.com"
    }
...
module "kube-hetzner" {
  ...
  # * Your ssh public key
  ssh_public_key = var.ssh_public_key != "" ? var.ssh_public_key : local.ssh_public_key
  ...
  extra_firewall_rules = [
    {
      description     = "To Allow Cloudflared to setup the tunnel to Cloudflare (UDP)"
      direction       = "out"
      protocol        = "udp"
      port            = "7844"
    ...
    {
      description     = "To Allow Cloudflared to setup the tunnel to Cloudflare (TCP)"
      direction       = "out"
      protocol        = "tcp"
      port            = "7844"
      source_ips      = [] # Won't be used for this rule
    ...
    {
      description     = "To Allow ArgoCD to fetch git repositories (TCP)"
      direction       = "out"
      protocol        = "tcp"
      port            = "22"
      source_ips      = [] # Won't be used for this rule
